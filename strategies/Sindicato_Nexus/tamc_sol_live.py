...
# NOTE: Debe existir el modelo entrenado
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("tamc_sol_live.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("TAMC_SOL_Live_V2")

class TAMC_SOL_BotLive:
    def __init__(self):
    ...
    def _init_exchange(self):
        try:
            ...
            # --- MODO L I V E ---
            # exchange.set_sandbox_mode(True) -> REMOVED FOR LIVE TRADING
            logger.warning("🚨 MODO LIVE ACTIVADO 🚨 - USANDO DINERO REAL EN HYPERLIQUID MAINNET")
    ...
    def get_model_action(self, df_1h):
        try:
            ...
            df_ind = align_daily_features(df_ind, self.config.ticker, "1mo")
            
            if len(df_ind) < self.config.seq_length:
                return 0 
                
            current_pos = self.get_position()
            pos_val, unrealized_pnl = 0.0, 0.0
    ...
    def execute_trade(self, action_id, current_price, current_candle_time=None):
        ...
        current_pos = self.get_position()
        current_size, current_side = 0.0, None
        ...
        amount = round(trade_value / current_price, 2)
        if amount <= 0: return
    ...
    def run(self):
        logger.warning("--- BOT TAMC 2.0 SOL INICIADO EN HYPERLIQUID *MAINNET LIVE* ---")
        ...
        while True:
            try:
                ...
                logger.info(f"Time: {last_time} | Price: {current_price:.2f} | IA: {self.action_names[action_id]}")
